...
def load_video_frame(frames_folder, frame_height=27, frame_width=48, fps=30):
    # Get the list of frame files and sort them
    frame_files = sorted([f for f in os.listdir(frames_folder) if f.endswith('.jpg')])
    num_frames = len(frame_files)

    # Initialize a tensor to hold the video frames
    input_video = torch.zeros(1, num_frames, frame_height, frame_width, 3, dtype=torch.uint8)
...
def main(args):
    ...
    for video_name in progress_bar:
        ...
        frames = load_video_frame(frame_folder, fps=args.fps)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--threshold", type=float, default=0.5, help="Threshold for scene change detection. Default 0.5")
    parser.add_argument("--fps", type=int, default=30, help="FPS of the video. Default 30")
